import pygame
import katebuton #seperate file that runs Kate's button class
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 0, 0)
PURPLE = (100,0,200)
...
class Piano:
    ...
    def create(self):
        ...
        variable is used to fix the positions of the black keys of where the notes are in the Keys class. Generating the white keys are easy first and you generate
        the white keys first because the black keys would get covered if they were with or first.'''
        w = 0
        b = 0
        rotate = 0
        switch = False
...
class Keys(Piano):
    def __init__(self,num,value,calibrate):
        ...
        if self.number in self.white_keys: #Length and Width of a white key and the position
            ...
            if self.number > 12:
                self.kbutton = katebuton.Button(self.length,self.width,self.xpos,self.ypos,self.white_key_notes[self.white_keys.index(self.number)].capitalize(),True,self.white_key_notes[self.white_keys.index(self.number)].capitalize(),WHITE)#Length, width, x,y, text, visbility, Key assignment, Colour
            else:
                self.kbutton = katebuton.Button(self.length,self.width,self.xpos,self.ypos,self.white_key_notes[self.white_keys.index(self.number)],True,self.white_key_notes[self.white_keys.index(self.number)],WHITE)#Length, width, x,y, text, visbility, Key assignment, Colour
            ...
            if self.number > 12:
                self.kbutton = katebuton.Button(self.length,self.width,self.xpos,self.ypos,self.black_key_notes[self.calibrate].capitalize(),True,self.black_key_notes[self.calibrate].capitalize(),BLACK)#Length, width, x,y, text, visbility, Key assignment, Colour
            else:
                self.kbutton = katebuton.Button(self.length,self.width,self.xpos,self.ypos,self.black_key_notes[self.calibrate],True,self.black_key_notes[self.calibrate],BLACK)#Length, width, x,y, text, visbility, Key assignment, Colour
    ...
    def Sound(self,state):
        ...
        if self.check == 0:
            ...
            if state == "y":
                self.status = True
                self.check += 1
                pygame.mixer.Channel(10).play(pygame.mixer.Sound("Piano Notes\Octave 2\G#5.mp3"))
            if state == "Y":
                self.status = True
                self.check += 1
